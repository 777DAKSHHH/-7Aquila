...
/**
 * Custom hook managing the Writing Task 2 Timer Engine lifecycle.
 */
export const useTask2Timer = ({
  sessionId,
  initialTimeSeconds = 0,
  duration = TASK2_DEFAULTS.TIME_LIMIT_SECONDS, // 2400 seconds (40 min)
  onExpire,
}) => {
    ...
    async (currentRemaining) => {
      ...
      try {
        await SessionService.updateWritingSession(sessionId, {
          task2_time_seconds: elapsedSeconds,
          total_time_seconds: elapsedSeconds,
        });
      } catch (err) {
        console.error("useTask2Timer Database Sync Error:", err);
      }
    },
    [sessionId, duration]
  );
